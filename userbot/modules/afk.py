...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**Pergi Hilang Dan OPLEN :3**\
        \nALESAN : `{string}`")
    else:
        await afk_e.edit("**Pergi Hilang Dan OPLEN :3**")
    if BOTLOG:
...
@register(outgoing=True, pattern="^.p(?: |$)(.*)", disable_errors=True)
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.edit("**Datang Muncul Dan ONLEN :3**")
        time.sleep(3)
        await msg.delete()
        if BOTLOG:
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**Pergi Hilang Dan OPLEN :3** (Since **{afk_since}**).\
                        \nALESAN : `{AFKREASON}`")
                else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"**Pergi Hilang Dan OPLEN :3** (Since **{afk_since}**).\
                        \nALESAN : `{AFKREASON}`")
                else:
